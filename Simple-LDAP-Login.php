		...
		if( $auth_result ) {
			...
			if( $this->user_has_groups( $username, $this->get_setting('directory') ) ) {
				...
				if ( ! $user || ( strtolower($user->user_login) !== strtolower($username) ) )  {
                    ...
                    //echo "  exit()  ";
                    //exit();
                    $new_user = wpmu_create_user($username, $password,$username);
					...
					if( ! is_wp_error($new_user) )
					{
						...
						$newdomain = 'blog.deu.edu.tr';
						//$newdomain = 'localhost';
						$path='/' . $fname .'-' . $lname;

						$blog_id=wpmu_create_blog($newdomain, $path, "no title", $new_user, array('public' => 1));
						$new_user = new WP_User($new_user,$username, $blog_id);
